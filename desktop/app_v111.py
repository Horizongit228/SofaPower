...
def adapters():
    data = jps(
        ...
        "Select Name,InterfaceDescription,MacAddress,Status,ifIndex,InterfaceGuid | ConvertTo-Json -Compress"
    )
    if not data:
        return []
...
def _state(value):
    ...
    if not text:
        return None
    enabled = ('enabled', 'enable', 'on', 'true', '1', 'включено', 'вкл', 'активно', 'разрешено')
    disabled = ('disabled', 'disable', 'off', 'false', '0', 'выключено', 'выкл', 'отключено', 'запрещено')
    if any(text == x or text.startswith(x + ' ') for x in enabled):
        return True
    if any(text == x or text.startswith(x + ' ') for x in disabled):
        return False
    return None


def advanced_magic_state(items):
    if not items:
        return None
    explicit_false = False
    for item in items:
        key = str(item.get('RegistryKeyword', '') or '').lower()
        name = str(item.get('DisplayName', '') or '').lower()
        hay = f'{key} {name}'
        if not any(token in hay for token in ('magic', 'wakeonmagic', 'shutdownwake', 's5wake', 'magic packet', 'магичес')):
            continue
        value = item.get('RegistryValue')
        display = item.get('DisplayValue')
        if value in (1, [1], '1', ['1']) or _state(display) is True:
            return True
        if value in (0, [0], '0', ['0']) or _state(display) is False:
            explicit_false = True
    if explicit_false:
        return False
    return None
...
def registry_magic_state(items):
    if not items:
        return None
    explicit_false = False
    for item in items:
        name = str(item.get('Name', '') or '').lower()
        if not any(token in name for token in ('wakeonmagic', 'shutdownwake', 's5wake', 'magicpacket', 'pme')):
            continue
...
def effective_magic(info):
    direct = _state(info.get('magic'))
    if direct is not None:
        return direct, 'Windows'
    advanced = advanced_magic_state(info.get('advanced', []))
    if advanced is True:
        return True, 'драйвер'
    registry = registry_magic_state(info.get('registry', []))
    if registry is True:
        return True, 'реестр драйвера'
    if advanced is False or registry is False:
...
def adapter_details(adapter):
    ...
    power = jps(
        ...
        "[PSCustomObject]@{"
        "Magic=$(if($p -and $null -ne $p.WakeOnMagicPacket){[string]$p.WakeOnMagicPacket}else{'Unknown'});"
        "Pattern=$(if($p -and $null -ne $p.WakeOnPattern){[string]$p.WakeOnPattern}else{'Unknown'})"
    ...
    advanced = jps(
        ...
        "Where-Object {"
        "$_.RegistryKeyword -match '(?i)wake|magic|pme|shutdown|s5' -or "
        "$_.DisplayName -match '(?i)wake|magic|пробуж|магичес|пакет'"
    ...
    registry = jps(
        ...
        "$g=$(if($a){([string]$a.InterfaceGuid).Trim('{}')}else{''});"
        "$root='HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e972-e325-11ce-bfc1-08002be10318}';"
        "$out=@();"
        "if($g){Get-ChildItem $root -ErrorAction SilentlyContinue | ForEach-Object {"
        "$p=Get-ItemProperty $_.PSPath -ErrorAction SilentlyContinue;"
        "if(([string]$p.NetCfgInstanceId).Trim('{}') -ieq $g){"
        "$p.PSObject.Properties | Where-Object {$_.Name -match '(?i)wake|magic|pme|shutdown|s5'} | "
        "ForEach-Object {$out += [PSCustomObject]@{Name=$_.Name;Value=[string]$_.Value}}"
        "}}};"
...
def configure(adapter, disable_fast=True):
    name = str(adapter.get('Name', '')).replace("'", "''")
    desc = str(adapter.get('InterfaceDescription', '')).replace("'", "''")
    disable_fast_ps = '$true' if disable_fast else '$false'
    token = f'{os.getpid()}-{int(time.time())}'
    script_path = Path(tempfile.gettempdir()) / f'SofaPowerHub-config-{token}.ps1'
    status_path = Path(tempfile.gettempdir()) / f'SofaPowerHub-status-{token}.json'
    status_ps = str(status_path).replace("'", "''")

    script = f"""
$n='{name}'
$d='{desc}'
$status='{status_ps}'
$disableFast={disable_fast_ps}
$errors=@()
$ErrorActionPreference='Stop'

try {{
    Set-NetAdapterPowerManagement -Name $n -WakeOnMagicPacket Enabled -WakeOnPattern Disabled -ErrorAction Stop
}} catch {{ $errors += 'PowerManagement: ' + $_.Exception.Message }}

try {{
    $props = Get-NetAdapterAdvancedProperty -Name $n -AllProperties -ErrorAction Stop | Where-Object {{
        $_.RegistryKeyword -match '(?i)WakeOnMagicPacket|ShutdownWake|S5Wake|MagicPacket' -or
        $_.DisplayName -match '(?i)magic packet|wake on magic|магичес.*пакет'
    }}
    foreach($p in $props) {{
        try {{
            Set-NetAdapterAdvancedProperty -Name $n -RegistryKeyword $p.RegistryKeyword -RegistryValue 1 -NoRestart -ErrorAction Stop
        }} catch {{ $errors += 'Advanced ' + $p.RegistryKeyword + ': ' + $_.Exception.Message }}
    }}
}} catch {{ $errors += 'AdvancedProperties: ' + $_.Exception.Message }}

try {{ powercfg /deviceenablewake \"$d\" | Out-Null }} catch {{ $errors += 'WakeArmed: ' + $_.Exception.Message }}

if($disableFast) {{
    try {{
        Set-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Power' -Name HiberbootEnabled -Type DWord -Value 0 -Force -ErrorAction Stop
    }} catch {{ $errors += 'FastStartup: ' + $_.Exception.Message }}
}}

try {{ Restart-NetAdapter -Name $n -Confirm:$false -ErrorAction Stop }} catch {{ $errors += 'RestartAdapter: ' + $_.Exception.Message }}
Start-Sleep -Seconds 1

$result=[PSCustomObject]@{{Ok=$true;Errors=$errors}}
$result | ConvertTo-Json -Compress | Set-Content -LiteralPath $status -Encoding UTF8
try {{ Remove-Item -LiteralPath $MyInvocation.MyCommand.Path -Force }} catch {{}}
"""

    script_path.write_text(script, encoding='utf-8-sig')
    result = ctypes.windll.shell32.ShellExecuteW(
        None, 'runas', 'powershell.exe', f'-NoProfile -ExecutionPolicy Bypass -File "{script_path}"', None, 0
    )
    if result <= 32:
        try:
...
class Hub(ctk.CTk):
    ...
    def build(self):
        ...
        ctk.CTkLabel(left, text='Hub проверяет физический Ethernet и учитывает особенности драйверов Realtek.', text_color=MUTED, font=ctk.CTkFont(size=12), wraplength=560, justify='left').grid(row=1, column=0, sticky='w', padx=22, pady=(0, 14))
    ...
    def apply_details(self, adapter, info):
        ...
        magic_state, magic_source = effective_magic(info)
        if magic_state is True:
            magic_text = 'Enabled' if magic_source == 'Windows' else f'Enabled ({magic_source})'
            magic_color = OK
        elif magic_state is False:
            magic_text = 'Disabled'
            magic_color = WARN
        else:
            magic_text = 'Не определено'
            magic_color = MUTED
        self.vwol.configure(text=magic_text, text_color=magic_color)
        ...
        checks = [
            True,
            str(adapter.get('Status', '')).lower() == 'up',
            normalize_mac(adapter.get('MacAddress', '')) is not None,
            bool(info.get('ip')),
            magic_state,
            advanced,
            bool(info.get('wake_armed')),
            fast == '0',
        ]
        ...
        if solid == len(checks):
            ...
            if magic_source != 'Windows':
                self.say('WoL готов ✓ Realtek сообщает Magic Packet через параметры драйвера — это нормально.', OK)
            else:
                self.say('Windows выглядит готовой к Wake-on-LAN. BIOS остаётся проверить вручную.', OK)
        elif solid == known and known < len(checks):
            self.summary.configure(text=f'Готово {solid}; ещё {len(checks) - known} пункт(а) драйвер не раскрывает', text_color=OK)
            self.say('Доступные проверки пройдены. Некоторые драйверы Realtek не отдают все флаги Windows API.', OK)
        else:
            self.summary.configure(text=f'Готово {solid} из {known} проверяемых пунктов', text_color=WARN)
            self.say('Нажми «Настроить Wake-on-LAN», чтобы Hub применил доступные параметры Windows.', WARN)
    ...
    def setup(self):
        if not self.current:
            return
        try:
            status_path = configure(self.current, bool(self.fast.get()))
            if status_path is None:
                self.say('Запрос прав администратора отменён.', WARN)
                return
            self.go.configure(state='disabled', text='Применяю…')
            self.say('Подтверди UAC. Hub дождётся завершения и сам перепроверит настройки.', OK)
            threading.Thread(target=self.wait_setup, args=(status_path,), daemon=True).start()
        except Exception as error:
            self.say(f'Не удалось запустить настройку: {error}', WARN)

    def wait_setup(self, status_path):
        ...
        while time.time() < deadline:
            ...
            time.sleep(0.4)
        if data is None:
            self.after(0, lambda: self.say('Настройка не завершилась за 30 секунд. Нажми «Обновить».', WARN))
            self.after(0, lambda: self.go.configure(state='normal', text='⚡ Настроить Wake-on-LAN'))
            return
        errors = data.get('Errors') or []
        if isinstance(errors, str):
            errors = [errors]
        if errors:
            self.after(0, lambda: self.say('Настройки применены частично; перепроверяю реальные флаги драйвера…', WARN))
        else:
            self.after(0, lambda: self.say('Настройки применены ✓ Перепроверяю адаптер…', OK))
